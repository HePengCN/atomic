...
int  xstack_push(hstack_t stack, const void* pdata, uint32_t data_size)
{
    ...
    memcpy(new_node->data, pdata, data_size);
...
@return if ok: true;
        if fail: false;
*************************************************/
bool  xstack_isempty(hstack_t stack)
{
    assert(stack != NULL);
    xstack_t* hstack = (xstack_t*)stack;
    bool ret = true;
