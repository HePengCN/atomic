...
int main(int argc, char* argv[])
{
    ...
    hThd_t* mcu_recv = (hThd_t*)mcurecv_thd_handle_new("mcurecv_0", 3000, queue_mcu2cpu);
    if(NULL == mcu_recv) {
    ...
    printf("%s, %d: thd_create ret: %d, mcu_recv->state: %s\n", __FUNCTION__, __LINE__, ret, thd_state_str(thd_state(mcu_recv)));

#if 1
    ret = thd_start(mcu_recv);
    printf("%s, %d: thd_start ret: %d, mcu_recv->state: %s\n", __FUNCTION__, __LINE__, ret, thd_state_str(thd_state(mcu_recv)));

    ret = thd_stop(mcu_recv);
    printf("%s, %d: thd_stop ret: %d,  mcu_recv->state: %s\n", __FUNCTION__, __LINE__, ret, thd_state_str(thd_state(mcu_recv)));
    ...
    printf("%s, %d: thd_destory ret: %d, mcu_recv->state: %s\n", __FUNCTION__, __LINE__, ret, thd_state_str(thd_state(mcu_recv)));
#endif

    ret = thd_join(mcu_recv);
    printf("%s, %d: thd_join ret: %d, mcu_recv->state: %s\n", __FUNCTION__, __LINE__, ret, thd_state_str(thd_state(mcu_recv)));
